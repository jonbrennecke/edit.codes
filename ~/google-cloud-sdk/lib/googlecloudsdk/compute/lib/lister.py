...
def BatchList(service, requests, http, batch_url, errors):
  """Makes a series of batch requests.

  If any response leads to an exception, that exception is raised.

  Args:
    service: An object with a List method that can list Compute
      Engine zone resources.
    requests: A list of protocol buffer objects that represent
      API requests to Compute Engine.
  ...
  """
  while requests:
    batch_requests = [(service, 'List', request) for request in requests]
...
def GetZonalResources(zones_service, resource_service, project, requested_zones,
  ...
  if requested_zones:
    ...
    zones = list(BatchList(
        service=zones_service,
        requests=[zones_service.GetRequestType('List')(project=project)],
        http=http,
        batch_url=batch_url,
        errors=errors))
  ...
  for zone_name in zone_names:
    list_requests.append(resource_service.GetRequestType('List')(
...
def GetRegionalResources(regions_service, resource_service, project,
  ...
  if requested_regions:
    ...
    regions = BatchList(
        service=regions_service,
        requests=[regions_service.GetRequestType('List')(project=project)],
        http=http,
        batch_url=batch_url,
        errors=errors)
...
def GetGlobalResources(resource_service, project, requested_name_regexes, http,
  ...
  list_requests = [resource_service.GetRequestType('List')(
      filter=filter_expression,
      project=project,
      maxResults=constants.MAX_RESULTS_PER_PAGE)]
