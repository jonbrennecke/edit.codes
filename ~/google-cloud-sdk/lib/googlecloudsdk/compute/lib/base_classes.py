...
class BaseLister(BaseResourceFetcher):
  ...
  def Args(parser):
    ...
    l.detailed_help = """\
        If provided, prints a human-readable table of useful resource
        data. The columns of the table do not necessarily map
        one-to-one to the properties defined in the API. For example,
        the external IP address of an instance is displayed under the
        ``externalIP'' column, but in the API the external IP address
        is stored under
        ``networkInterfaces[0].accessConfigs[0].natIP''. The table
        output is for convenience and should not be taken to
        reflect the API verbatim.
        """
...
class BaseDeleter(BaseAsyncMutator):
  ...
  def path_handler(self):
    return self.context['path-handler']
  ...
  def CreateRequests(self, args):
    ...
    for name in args.name:
      requests.append(self.CreateRequestMessage(
          args=args,
          delete_request_class=delete_request_class,
  ...
  def Run(self, args):
    if not console_io.PromptContinue(
        message='The following {0} will be deleted: {1}'.format(
            self.collection, ', '.join(args.name))):
      raise calliope_exceptions.ToolException('deletion aborted by user')

    return super(BaseDeleter, self).Run(args)
...
class ZonalDeleter(BaseDeleter):
  ...
  def CreateRequestMessage(self, args, delete_request_class, name_field, name):
    resource_context = self.path_handler.Parse(
        self.collection, name, zone=args.zone)

    request = delete_request_class(
        project=resource_context['project'],
        zone=resource_context['zone'])
    setattr(request, name_field, resource_context[name_field])
    return request
...
class RegionalDeleter(BaseDeleter):
  ...
  def CreateRequestMessage(self, args, delete_request_class, name_field, name):
    resource_context = self.path_handler.Parse(
        self.collection, name, region=args.region)

    request = delete_request_class(
        project=resource_context['project'],
        region=resource_context['region'])
    setattr(request, name_field, resource_context[name_field])
    return request
...
class GlobalDeleter(BaseDeleter):
  ...
  def CreateRequestMessage(self, args, delete_request_class, name_field, name):
    resource_context = self.path_handler.Parse(self.collection, name)
    request = delete_request_class(project=resource_context['project'])
    setattr(request, name_field, resource_context[name_field])
    return request
...
class ReadWriteCommand(BaseCommand):
  ...
  def GetContext(self, args):
    """Gets the context for this resource."""
  ...
  def Run(self, args):
    self.resource_context = self.GetContext(args)
...
class ProjectMetadataMutatorMixin(ReadWriteCommand):
  ...
  def GetContext(self, unused_args):
    return None
...
class BaseEdit(ReadWriteCommand):
  ...
  def Modify(self, args, existing):
    # Shortens resource references if --raw-links is not provided.
    reference_simplifier = property_selector.PropertySelector(
        transformations=None if args.raw_links else self.transformations)
    transformed_original_record = reference_simplifier.Apply(
        encoding.MessageToDict(existing))

    # Selects only the fields that can be modified.
    field_selector = property_selector.PropertySelector(
        properties=self._resource_spec.editables)
    modifiable_record = field_selector.Apply(transformed_original_record)
    ...
    buf.write('\n')
    buf.write(_SerializeDict(modifiable_record, args.format))
    buf.write('\n')

    example = _SerializeDict(
        reference_simplifier.Apply(
    ...
    original = _SerializeDict(transformed_original_record, args.format)
    ...
    while True:
      ...
      try:
        ...
        if isinstance(e, ValueError):
          ...
          message = str(e)
        if not console_io.PromptContinue(
            message=('There was a problem parsing your changes: {0}'
                     .format(message)),
    ...
    modifiable_record = reference_normalizer.Apply(modifiable_record)
    ...
    if modifiable_record == modified_record:
      ...
      modified_record['name'] = transformed_original_record['name']
      fingerprint = transformed_original_record.get('fingerprint')
      if fingerprint:
