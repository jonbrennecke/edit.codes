...
class DeprecateImages(base_classes.BaseAsyncMutator):
  ...
  def Args(parser):
    ...
    replacement.detailed_help = """\
       ...
       ``--replacement projects/google/global/images/my-image''. This flag
       is required when setting the image state to anything other than
       ``ACTIVE'' or when --delete-in, --delete-on, --deprecate-in,
       --deprecate-on, --obsolete-in, or --obsolete-on is provided.
       """
    ...
    deprecate_on = deprecate_group.add_argument(
        '--deprecate-on',
        help=('Specifies the date and time when the state of this image '
              'will become DEPRECATED.'))
    deprecate_on.detailed_help = """\
       Specifies time (in the same format as --delete-on) when this image's
       status should become DEPRECATED.
       This flag is mutually exclusive with --deprecate-in.
       """

    deprecate_in = deprecate_group.add_argument(
        '--deprecate-in',
        help=('Specifies the amount of time until this image should become '
              'DEPRECATED.'),
        type=arg_parsers.Duration())
    deprecate_in.detailed_help = """\
       Specifies time (in the same format as --delete-in) until this image's
       status should become DEPRECATED, which will issue a warning and suggest
       a replacement image whenever the image is applied to a new disk.
  ...
  def CreateRequests(self, args):
    ...
    if (any([args.delete_on, args.delete_in, args.deprecate_on,
             args.deprecate_in, args.obsolete_on, args.obsolete_in,
             args.replacement]) and args.state == 'ACTIVE'):
      raise calliope_exceptions.ToolException(
          'if the state is set to ACTIVE then none of --delete-on, '
          '--delete-in, --deprecate-on, --deprecate-in, --obsolete-on, '
          '--obsolete-in, or --replacement may be provided')
